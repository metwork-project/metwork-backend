...
def generate_network(
    scores_matrix, mzs, top_k, pairs_min_cosine, use_self_loops=False
):  # TODO: change this in worker
    interactions = []
    num_spectra = len(mzs)
    ...
    for i, (x, y, _, _) in enumerate(interactions):
        x_ind = np.where(
            np.logical_or(interactions["Source"] == x, interactions["Target"] == x)
        )[0][:top_k]
        y_ind = np.where(
            np.logical_or(interactions["Source"] == y, interactions["Target"] == y)
        )[0][:top_k]
        if (
            x in interactions[y_ind]["Source"] or x in interactions[y_ind]["Target"]
        ) and (
            y in interactions[x_ind]["Source"] or y in interactions[x_ind]["Target"]
        ):
            mask[i] = True
