...
class FragCompare:
    ...
    def compare_frag_mols(self, frag_mols):
        ...
        energies = [
            [
                fms[0]
                for fms in fm.fragmolspectrum_set.values_list("energy")
                .order_by("energy")
                .distinct()
            ]
            for fm in frag_mols
        ...
        for e0 in energies[0]:
            for e1 in energies[1]:
                ...
                if cosine > best_cosine:
                    ...
                    best_energies = (e0, e1)
        if best_energies:
            best_energies = "{0}:{1},{2}:{3}".format(
                frag_mols[0].id, best_energies[0], frag_mols[1].id, best_energies[1]
            )
    ...
    def evaluate_match(self, frag_mol_compare):
        frag_mol_compare.match = (
            frag_mol_compare.cosine >= self.frag_compare_conf.cosine_threshold
        )
    ...
    def get_cosine(self, params):
        ...
        spectrum = [p[0].fragmolspectrum_set.get(energy=p[1]).spectrum for p in params]
