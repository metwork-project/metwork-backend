...
class MetGraph:
    def __init__(self, project):
        ...
        self.rps = [
            rp
            for rp in project.react_processes.annotate(Count("products"))
            .filter(products__count__gt=0)
            .filter(
                Q(products__in=project.molecules_matching())
                | Q(products__in=self.mols_in)
            )
        ]
        ...
        self.mols_mass = {
            mol.id: mol.mass_exact() + self.get_adduct_mass(mol) for mol in self.mols
        }
        self.mols_dic = {
    ...
    def products(self, rp):
        return rp.products.filter(
            Q(id__in=self.project.molecules_matching()) | Q(id__in=self.mols_in)
        ).distinct()
    ...
    def gen_metexplore(self):
        ...
        nodes = [
            {"name": rp.reaction.name, "biologicalType": "reaction"} for rp in self.rps
        ]
        nodes += [
            {
                "name": str(round(m.mass_exact(), 3)),
