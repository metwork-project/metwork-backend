...
class ChemDoodle(object):
        ...
        for atom in mw.GetAtoms():
            if atom.GetSymbol() == "C":
                ...
                for b in atom.GetBonds():
                    id_1 = "{0}-{1}".format(b.GetBeginAtomIdx(), b.GetEndAtomIdx())
                    if id_1 in bonds_cd:
                        b_cd = json_data["b"][bonds_cd[id_1]]
                    else:
                        b_cd = json_data["b"][
                            bonds_cd[
                                "{1}-{0}".format(b.GetBeginAtomIdx(), b.GetEndAtomIdx())
                            ]
                        ]
                    if "s" in b_cd:
                        s.append(b_cd["s"])
                ...
                if len(s) == 4 and s.count("protruding") * s.count("recessed") == 1:
                    ...
                    int = Line(points[2].midpoint(points[3]), points[1]).intersection(
                        Line(points[1].midpoint(points[2]), points[3])
                    )[0]
                    N = CoordSys3D("N")
                    coords = [N.i, N.j, N.k]
                    vectors = []
    ...
    def mol_rdkit_to_json(self, mr, begin_id={"a": 0, "b": 0}):
    ...
    def react_to_json(self, reaction):
        ...
        for i, map in atom_maping.items():
            json_res["s"].append(
                {"i": "s{0}".format(i), "t": "AtomMapping", "a1": map[0], "a2": map[1]}
            )
