...
for constructor in [
    ...
    _F = type(
        str(constructor.upper()),
        (ConstructorFunc,),
        {"function": constructor, "default_output_field": MolField(),},
    )
    setattr(module, _F.__name__, _F)
    __all__.append(_F.__name__)
...
for validator in ["is_valid_smiles", "is_valid_smarts", "is_valid_ctab"]:
    _F = type(
        str(validator.upper()),
        (ValidatorFunc,),
        {"function": validator, "default_output_field": models.BooleanField(),},
    )
    setattr(module, _F.__name__, _F)
    __all__.append(_F.__name__)
