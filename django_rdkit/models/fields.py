...
class MolField(Field):
    ...
    def get_prep_lookup(self, lookup_type, value):
        ...
        supported_lookup_types = ["hassubstruct", "issubstruct", "exact", "isnull",] + [
...
MOL_DESCRIPTOR_MIXINS = [
    make_descriptor_mixin(d, "mol", fieldkls()) for d, fieldkls in MOL_DESCRIPTORS
]


MOL_DESCRIPTOR_TRANFORMS = [
    type(
        str("{0}_Transform".format(mixin.descriptor_name.upper())),
...
RXN_DESCRIPTOR_MIXINS = [
    make_descriptor_mixin(d, "reaction", fieldkls()) for d, fieldkls in RXN_DESCRIPTORS
]
